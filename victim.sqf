...
if(side _killer == EAST and !isPlayer _killer) exitwith
	...
	format['["Died", "%1"] spawn Isse_AddCrimeLogEntry;kopfgeld_%1 = 0;server globalChat "%2 was killed by the South Takistan Liberation Army!"', player, name player] call broadcast;

	};
	
if(side _killer == WEST and !isPlayer _killer) exitwith

	{

	format['["Died", "%1"] spawn Isse_AddCrimeLogEntry;kopfgeld_%1 = 0;server globalChat "%2 was killed by the UN Stabilization Forces!"', player, name player] call broadcast;

	};
	
if(side _killer == RESISTANCE and !isPlayer _killer) exitwith

	{

	format['["Died", "%1"] spawn Isse_AddCrimeLogEntry;kopfgeld_%1 = 0;server globalChat "%2 was killed by the Insurgents!"', player, name player] call broadcast;
