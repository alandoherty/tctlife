...
if (_moneh > _bail) exitwith
{
	['moneh', -(_bail)] call INV_AddInventoryItem;
	player groupChat format [localize "STRS_bail_self", (_bail call ISSE_str_IntToStr)];
	format ["if (iscop) then {[""moneh"", %2] call INV_AddInventoryItem; player groupChat format [localize ""STRS_bail_addbail"", %2, ""%3""];}; titletext [format[localize ""STRS_bail_payed"",%3, %1], ""plain""]; kopfgeld_%4 = kopfgeld_%4 - %1; if(kopfgeld_%4 < 0)then{kopfgeld_%4 = 0};", (_bail call ISSE_str_IntToStr), (_copbail call ISSE_str_IntToStr), rolenumber, player] call broadcast;
};


if (bankaccount > _bail) exitWith
{
	bankaccount = bankaccount - _bail;
	player groupChat format [localize "STRS_bail_self", (_bail call ISSE_str_IntToStr)];
	format ["if (iscop) then {[""moneh"", %2] call INV_AddInventoryItem; player groupChat format [localize ""STRS_bail_addbail"", %2, ""%3""];}; titletext [format[localize ""STRS_bail_payed"",%3, %1], ""plain""]; kopfgeld_%4 = kopfgeld_%4 - %1; if(kopfgeld_%4 < 0)then{kopfgeld_%4 = 0};", (_bail call ISSE_str_IntToStr), (_copbail call ISSE_str_IntToStr), rolenumber, player] call broadcast;
};
