...
if(str(_shop) == "vipcar") then
{
    ...
    if(!_donator) exitwith {closedialog 0; player groupchat "You must be atleast a silver donator to access this store";}; 
};

if(str(_shop) == "pmccar") then
{
    _TCTmember = false;
    _uid = getPlayerUID player;

    if(_uid in armymember) then
    {
        _TCTmember = true;
    };
    
    if(!_TCTmember) exitwith {closedialog 0; player groupchat "You must be a TCTmember to access this store";}; 
};

if(str(_shop) == "pmcbox") then
{
    _TCTmember = false;
    _uid = getPlayerUID player;
...
ctrlSetText [101,format [localize "STRS_inv_shopdialog_itemshop", ('moneh' call INV_GetItemAmount)]];
