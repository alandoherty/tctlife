...
for [{_c=0}, {_c < (count serverKontoArray)}, {_c=_c+1}] do
	...
	if (((serverKontoArray select _c) select 0) == (GetPlayerUID _Fplayer)) then {_Ffound = true;};

	};

if (!(_Ffound)) then {serverKontoArray = serverKontoArray + [ [ (GetPlayerUID _Fplayer) ] ];};

for [{_c=0}, {_c < (count serverKontoArray)}, {_c=_c+1}] do

	{

	if (((serverKontoArray select _c) select 0) == (GetPlayerUID _Fplayer)) then
