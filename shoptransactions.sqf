...
if(_moneh == 0)exitwith{player groupchat localize "STRS_inv_buyitems_keinmoneh"; _exitvar = 1};
...
if (_itemart == "Item" and instock) then
	...
	if (_moneh < _cost) then

		{

		_menge = (floor(_moneh/_CostMitTax));
		_cost  = _menge*_CostMitTax;
		if (_menge <= 0) exitWith {player groupChat localize "STRS_inv_buyitems_keinmoneh"; _exitvar = 1;};
	...
	if (_infos call INV_getitemIsIllegal and _cost > 0 and _infos call INV_getitemKindOf == "drug") then
		...
		for "_c" from 0 to (count gangsarray - 1) do
			...
			if(_control == _gang and count _members > 0)then
				...
				format['if(name player in %1)then{player groupchat "You received $%2 from a drug sale"; bankaccount = bankaccount + %2};', _members, _income] call broadcast;
...
if (((_itemart == "waffe") or (_itemart == "magazin")) and instock) then
	...
	if (!(_license1 call INV_HasLicense) and isciv and _license) exitWith {player groupChat format[localize "STRS_inv_buyitems_nolicense", (_license1 call INV_GetLicenseName)]; _exitvar = 1};
	if (!(_license2 call INV_HasLicense) and iscop and _license) exitWith {player groupChat format[localize "STRS_inv_buyitems_nolicensecop", (_license2 call INV_GetLicenseName)]; _exitvar = 1};
	if (_moneh < _cost) exitWith {player groupChat localize "STRS_inv_buyitems_keinmoneh"; _exitvar = 1};
	['moneh', -(_cost)] call INV_AddInventoryItem;
...
if (_itemart == "fahrzeug" and instock) then
	...
	if (_moneh < _CostMitTax) exitWith {player groupChat localize "STRS_inv_buyitems_keinmoneh"; _exitvar = 1};
	...
	['moneh', -(_CostMitTax)] call INV_AddInventoryItem;
...
if (_itemart == "fahrzeug") then
	...
	if ((_vehicle distance player) > 50) exitWith {player groupChat localize "STRS_inv_buyitems_sell_toofar"; _exitvar = 1};
	['moneh', (_CostMitTax)] call INV_AddInventoryItem;
