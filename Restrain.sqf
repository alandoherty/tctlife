...
if(animationstate _civ == "civillying01") exitwith
{
	...
	player groupchat "You released the civilian!";
	 ...
	 (format['if (rolestring == "%1") then {execVM "Unrestrain.sqf";}', _civ]) call broadcast;
};
...
(format['server globalchat "%3 was restrained by %2";if (rolestring == "%1") then _civ, name player, name _civ]) call broadcast;
